...
# In the system to be solved there are two strategies:
# 1) consider gamma_pv_0 as gamma_pv, meaning that the critical properties are derived for the initial conditions (meaning one single value of gamma_pv)
# 2) consider gamma_pv as variable, meaning that the critical properties are derived for each point along the nozzle (meaning a different value of gamma_pv for each point)

# moreover, the system is overdetermined now, because I added the mass flow equation
...
mass_flow_rate = np.zeros_like(area)
...
def mach_area_rel(x,y):
    ...
    # y = area
    # x[0] = Mach, x[1] = p, x[2] = D, x[3] = gamma_pv, x[4] = mass flow
    # considering gamma_pv_0 as gamma_pv
    # This because in Nedersigt 2023, the critical properties are derived for the initial conditions (meaning one single value of gamma_pv) 
    f1 = (y / A_star) - (1/x[0]) * ((2 + (gamma_pv_0 - 1) * x[0]**2)/(gamma_pv_0+1))**((gamma_pv_0+1) /2 /(gamma_pv_0 - 1) )
    # varying gamma_pv
    ...
    f5 = x[4] -  y * x[0] * np.sqrt(x[3] * dens0 * p0) * (x[1]/p0)**((1/2/x[3])+1)
...
for i, a in enumerate(area):
    ...
    if i == 0:
        x0 = [M0, p0, dens0, gamma_pv_0, mf]
    else:
        x0 = [M0, p[i-1], rho[i-1], gamma_pv[i-1], mass_flow_rate[i-1]]

    y = a
    mach, pp, Rr, gammaa_pv, masss_f = fsolve(mach_area_rel,x0, args=(a),xtol=1e-16,maxfev=10000)
    ...
    if mach - MACH[i-1] < 0: # force Mach to be higher than 1
        mach, pp, Rr, gammaa_pv, masss_f = fsolve (mach_area_rel, [2, p[i-1], rho[i-1], gamma_pv[i-1]], args=(a), xtol=1e-16, maxfev=10000)
